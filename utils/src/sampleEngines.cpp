...
#define SMP_RETVAL_IF_FALSE(condition, msg, retval, err)                                                               \
        ...
        switch (model.baseModel.format) {
            case ModelFormat::kCAFFE: {
                ...
                const auto blobNameToTensor = parser.caffeParser->parse(
                        model.prototxt.c_str(), modelName.empty() ? nullptr : modelName.c_str(), network,
                        DataType::kFLOAT);
                if (!blobNameToTensor) {
            ...
            case ModelFormat::kUFF: {
                ...
                for (const auto &s : model.uffInputs.inputs) {
                    if (!parser.uffParser->registerInput(
                            ...
                            model.uffInputs.NHWC ? UffInputOrder::kNHWC : UffInputOrder::kNCHW)) {
    ...
    namespace {
        ...
        public:
            RndInt8Calibrator(
                    int batches, std::vector<int> &elemCount, const std::string &cacheFile,
        ...
        RndInt8Calibrator::RndInt8Calibrator(
            ...
            for (int i = 0; i < network.getNbInputs(); i++) {
                ...
                cudaCheck(cudaMemcpy(data, rnd_data.data(), elemCount[i] * sizeof(float), cudaMemcpyHostToDevice),
                          mErr);
    ...
    ICudaEngine *networkToEngine(const BuildOptions &build, const SystemOptions &sys, IBuilder &builder,
        ...
        if ((build.int8 && build.calibration.empty()) || int8IO) {
            ...
            for (int i = 0; i < network.getNbInputs(); i++) {
                ...
                if (profileCalib) {
                    elemCount.push_back(
                            volume(profileCalib->getDimensions(input->getName(), OptProfileSelector::kOPT)));
                } else if (profile) {
    ...
    getEngine(const ModelOptions &model, const BuildOptions &build, const SystemOptions &sys, std::ostream &err) {
