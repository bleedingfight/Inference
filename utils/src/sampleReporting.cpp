...
// clang on
    ...
    void
    printEpilog(std::vector<InferenceTime> timings, float walltimeMs, float percentile, int queries, std::ostream &os) {
...
// clang off
        os << "Host Latency" << std::endl <<
           ...
           "mean: " << totalTime.latency() / timings.size() << " ms "
                                                               "(end to end " << totalTime.e2e / timings.size()
           << " ms)" << std::endl <<
...
// clang on
    ...
    void
    printPerformanceReport(const std::vector<InferenceTrace> &trace, const ReportingOptions &reporting, float warmupMs,
