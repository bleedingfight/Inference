...
namespace sample {
    ...
    bool setUpInference(InferenceEnvironment &iEnv, const InferenceOptions &inference) {
        ...
        if (nOptProfiles > 1) {
            sample::gLogWarning << "Multiple profiles are currently not supported. Running with one profile."
                                << std::endl;
        }
...
//!
        ...
        struct Enqueue {
            explicit Enqueue(nvinfer1::IExecutionContext &context, void **buffers) : mContext(context),
                                                                                     mBuffers(buffers) {}
...
//!
        ...
        public:
            ...
            explicit EnqueueImplicit(nvinfer1::IExecutionContext &context, void **buffers, int batch) : Enqueue(context,
                                                                                                                buffers),
                                                                                                        mBatch(batch) {}
...
//!
        ...
        public:
            ...
            explicit EnqueueExplicit(nvinfer1::IExecutionContext &context, void **buffers) : Enqueue(context,
                                                                                                     buffers) {}
...
//!
        ...
        public:
            ...
            Iteration(int id, const InferenceOptions &inference, nvinfer1::IExecutionContext &context,
                      Bindings &bindings) : mBindings(bindings),
                                            mStreamId(id), mDepth(1 + inference.overlap), mActive(mDepth),
                                            mEvents(mDepth), mEnqueueTimes(mDepth) {
        ...
        private:
            ...
            InferenceTrace getTrace(const TimePoint &cpuStart, const TrtCudaEvent &gpuStart, bool skipTransfers) {
                float is = skipTransfers ? getEvent(EventType::kCOMPUTE_S) - gpuStart : getEvent(EventType::kINPUT_S) -
                                                                                        gpuStart;
                float ie = skipTransfers ? getEvent(EventType::kCOMPUTE_S) - gpuStart : getEvent(EventType::kINPUT_E) -
                                                                                        gpuStart;
                float os = skipTransfers ? getEvent(EventType::kCOMPUTE_E) - gpuStart : getEvent(EventType::kOUTPUT_S) -
                                                                                        gpuStart;
                float oe = skipTransfers ? getEvent(EventType::kCOMPUTE_E) - gpuStart : getEvent(EventType::kOUTPUT_E) -
                                                                                        gpuStart;
                return InferenceTrace(mStreamId,
                                      std::chrono::duration<float, std::milli>(getEnqueueTime(true) - cpuStart).count(),
                                      std::chrono::duration<float, std::milli>(
                                              getEnqueueTime(false) - cpuStart).count(),
        ...
        void inferenceLoop(IterationStreams &iStreams, const TimePoint &cpuStart, const TrtCudaEvent &gpuStart,
                           int iterations, float maxDurationMs, float warmupMs, std::vector<InferenceTrace> &trace,
                           bool skipTransfers) {
            float durationMs = 0;
            int skip = 0;
        ...
        void
        inferenceExecution(const InferenceOptions &inference, InferenceEnvironment &iEnv, SyncStruct &sync, int offset,
            ...
            for (int s = 0; s < streams; ++s) {
                Iteration *iteration = new Iteration(offset + s, inference, *iEnv.context[offset],
                                                     *iEnv.bindings[offset]);
            ...
            inferenceLoop(iStreams, sync.cpuStart, sync.gpuStart, inference.iterations, durationMs, warmupMs,
                          localTrace, inference.skipTransfers);
        ...
        inline
        std::thread
        makeThread(const InferenceOptions &inference, InferenceEnvironment &iEnv, SyncStruct &sync, int thread,
                   int streamsPerThread, int device, std::vector<InferenceTrace> &trace) {
    ...
    void runInference(const InferenceOptions &inference, InferenceEnvironment &iEnv, int device,
