        ...
        for (const char &c : opt.longName) {
            if (!std::isalnum(c) && c != '-' && c != '_') {
                return "Long name '" + opt.longName +
                       "' contains characters that are not '-', '_', or "
                       "alphanumeric";
            }
        }
...
TRTParsedArgs parseArgs(int argc, const char *const *argv,
    ...
    for (int i = 1; i < argc; ++i) // index of current command-line argument
    {
        ...
        if (argStr.empty()) {
            return TRTParsedArgs{"Empty argument at index " +
                                 std::to_string(i)};
        }
        ...
        if (argStr == "-" || argStr == "--") {
            return TRTParsedArgs{
                "Argument does not specify an option at index " +
                std::to_string(i)};
        }
        ...
        if (options[idx].valueRequired) {
            if (!value.empty()) {
                ...
                parsedArgs.values[idx].first =
                    parsedArgs.values[idx].second.size();
                continue;
            }

            if (i + 1 >= argc) {
                return TRTParsedArgs{
                    "Last argument requires value, but none given"};
            }

            const std::string nextArg(argv[i + 1]);
            if (nextArg.size() >= 1 && nextArg[0] == '-') {
                sample::gLogWarning
                    << "Warning: Using '" << nextArg << "' as a value for '"
