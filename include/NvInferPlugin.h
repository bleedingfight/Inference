...
extern "C"
{
    ...
    //!
    TENSORRTAPI nvinfer1::IPluginV2* createRPNROIPlugin(int32_t featureStride, int32_t preNmsTop, int32_t nmsMaxOut,
    ...
    //!
    TENSORRTAPI nvinfer1::IPluginV2* createNormalizePlugin(
        const nvinfer1::Weights* scales, bool acrossSpatial, bool channelShared, float eps);
    ...
    //!
    TENSORRTAPI nvinfer1::IPluginV2* createAnchorGeneratorPlugin(
    ...
    //!
    //! \brief The LReLu plugin layer performs leaky ReLU for 4D tensors. Give an input value x, the PReLU layer
    //! computes the output as x if x > 0 and negative_slope //! x if x <= 0. Registered plugin type "LReLU_TRT".
    //! Registered plugin version "1". \param negSlope Negative_slope value.
    //!
    ...
    //!
    TENSORRTAPI nvinfer1::IPluginV2* createReorgPlugin(int32_t stride);
    ...
    //!
    TENSORRTAPI nvinfer1::IPluginV2* createSplitPlugin(int32_t axis, int32_t* output_lengths, int32_t noutput);
    ...
    //!
    TENSORRTAPI nvinfer1::IPluginV2* createInstanceNormalizationPlugin(
