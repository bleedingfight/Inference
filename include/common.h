...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                \
    do {                                                                       \
        if (!(status)) {                                                       \
            sample::gLogError << errMsg << " Error in " << __FILE__            \
...
#define ASSERT(condition)                                                      \
    do {                                                                       \
        if (!(condition)) {                                                    \
            sample::gLogError << "Assertion failure: " << #condition           \
                              << std::endl;                                    \
            abort();                                                           \
        }                                                                      \
    } while (0)
...
struct SimpleProfiler : public nvinfer1::IProfiler {
    ...
    friend std::ostream &operator<<(std::ostream &out,
        ...
        // Output header
        {
            out << std::setfill(' ') << std::setw(maxLayerNameLength)
                << layerNameStr << " ";
...
//! it might lie in. Function will also walk back MAX_DEPTH dirs from CWD to
//! check for such a file path.
...
inline T swapEndianness(const T &value) {
    ...
    for (int i = 0; i < static_cast<int>(sizeof(T)); ++i) {
        bytes[sizeof(T) - 1 - i] =
            *(reinterpret_cast<const uint8_t *>(&value) + i);
    }
...
class HostMemory {
  public:
    ...
    virtual nvinfer1::DataType type() const noexcept { return mType; }
    virtual ~HostMemory() {}
...
class TypedHostMemory : public HostMemory {
  public:
    ...
    ~TypedHostMemory() noexcept override { delete[] (ElemType *)mData; }
...
bool readASCIIFile(const std::string &fileName, const size_t size,
    ...
    std::ifstream infile(fileName);
    if (!infile.is_open()) {
        std::cout << "ERROR readASCIIFile: Attempting to read from a file that "
                     "is not open."
                  << std::endl;
        return false;
    }
...
bool writeASCIIFile(const std::string &fileName, const std::vector<T> &in) {
    ...
    if (!outfile.is_open()) {
        std::cout << "ERROR: writeASCIIFile: Attempting to write to a file "
                     "that is not open."
                  << std::endl;
        return false;
    }
...
inline int32_t calculateSoftmax(float *const prob, int32_t const numDigits) {
    ...
    ASSERT(sum != 0.0F);
    std::transform(prob, prob + numDigits, prob,
                   [sum](float v) -> float { return v / sum; });
...
// is not used. This function is just a utility to globally fill in missing
// scales and zero-points for the entire network.
//
...
// the network, cases where dynamic ranges are asymmetric.
//
// The default parameter values choosen arbitrarily. Range values should be
// choosen such that we avoid underflow or overflow. Also range value should be
// non zero to avoid uniform zero scale tensor.
...
inline void setDummyInt8DynamicRanges(const nvinfer1::IBuilderConfig *c,
    ...
    if (c->getFlag(nvinfer1::BuilderFlag::kINT8)) {
        sample::gLogWarning << "Int8 calibrator not provided. Generating dummy "
                               "per-tensor dynamic range. Int8 accuracy "
                               "is not guaranteed."
                            << std::endl;
...
inline void enableDLA(nvinfer1::IBuilder *builder,
                      nvinfer1::IBuilderConfig *config, int useDLACore,
    ...
    if (useDLACore >= 0) {
        if (builder->getNbDLACores() == 0) {
            std::cerr << "Trying to use DLA core " << useDLACore
                      ...
                      << std::endl;
            assert("Error: use DLA core on a platfrom that doesn't have any "
                   "DLA cores" &&
                   false);
        }
        if (allowGPUFallback) {
...
inline int64_t volume(nvinfer1::Dims const &dims, int32_t start, int32_t stop) {
    ...
    ASSERT(std::all_of(dims.d + start, dims.d + stop,
...
inline void readPPMFile(const std::string &filename, vPPM &ppm,
    ...
    ppm.fileName = filename;
    std::ifstream infile(locateFile(filename, input_dir),
                         std::ifstream::binary);
    infile >> ppm.magic >> ppm.w >> ppm.h >> ppm.max;
    infile.seekg(1, infile.cur);
...
class GpuTimer : public TimerBase {
  public:
    ...
    void start() override { CHECK(cudaEventRecord(mStart, mStream)); }
    void stop() override {
...
class DynamicLibrary {
  public:
    explicit DynamicLibrary(std::string const &name) : mLibName{name} {
#if defined(_WIN32)
        ...
        // asan doesn't handle module unloading correctly and there are no plans
        // on doing so. In order to get proper stack traces, don't delete the
        // shared library on close so that asan can resolve the symbols
        // correctly.
        flags |= RTLD_NODELETE;
#endif // ENABLE_ASAN
    ...
    std::function<Signature> symbolAddress(char const *name) {
        ...
        void *ret;
#if defined(_MSC_VER)
        ret = static_cast<void *>(
            GetProcAddress(static_cast<HMODULE>(mHandle), name));
#else
        ret = dlsym(mHandle, name);
#endif
        if (ret == nullptr) {
            std::string const kERROR_MSG(
                mLibName + ": error loading symbol: " + std::string(name));
    ...
    ~DynamicLibrary() {
        try {
            ...
            sample::gLogError << "Unable to close library: " << mLibName
                              << std::endl;
        }
    }
...
inline std::unique_ptr<DynamicLibrary> loadLibrary(std::string const &path) {
    // make_unique not available until C++14 - we still need to support C++11
    // builds.
...
class FileLock {
  public:
    FileLock(std::string const &fileName) : fileName(fileName) {
        std::string lockFileName = fileName + ".lock";
#ifdef _MSC_VER
        sample::gLogVerbose << "Trying to set exclusive file lock "
                            << lockFileName << std::endl;
        auto startTime = std::chrono::high_resolution_clock::now();
        // MS docs said this is a blocking IO if "FILE_FLAG_OVERLAPPED" is not
        // provided
        lock = CreateFileA(lockFileName.c_str(), GENERIC_WRITE, 0, NULL,
                           OPEN_ALWAYS, 0, NULL);
        if (lock != INVALID_HANDLE_VALUE) {
            float const time =
                std::chrono::duration<float>(
                    ...
                    .count();
            sample::gLogVerbose << "File locked in " << time << " seconds."
                                << std::endl;
        } else {
            throw std::runtime_error("Failed to lock " + lockFileName + "!");
        }
#elif defined(__QNX__)
        // We once enabled the file lock on QNX, lockf(F_TLOCK) return -1 and
        // the reported error is The error generated was 89 That means :
    ...
    ~FileLock() {
...
#ifdef _MSC_VER
        if (lock != INVALID_HANDLE_VALUE) {
            sample::gLogVerbose << "Trying to remove exclusive file lock "
                                << lockFileName << std::endl;
            auto startTime = std::chrono::high_resolution_clock::now();
            CloseHandle(lock);
            float const time =
                std::chrono::duration<float>(
                    std::chrono::high_resolution_clock::now() - startTime)
                    .count();
            sample::gLogVerbose << "File unlocked in " << time << " seconds."
                                << std::endl;
        }
#elif defined(__QNX__)
        // We once enabled the file lock on QNX, lockf(F_TLOCK) return -1 and
        ...
        if (fd != -1) {
            ...
            if (ret != 0) {
                sample::gLogVerbose << "Failed to unlock " << lockFileName
                                    << "!" << std::endl;
            } else {
                fd = -1;
                fclose(fp);
                float const time =
                    std::chrono::duration<float>(
                        std::chrono::high_resolution_clock::now() - startTime)
                        .count();
                sample::gLogVerbose << "File unlocked in " << time
                                    << " seconds." << std::endl;
            }
        }
#endif
...
inline std::vector<char> loadTimingCacheFile(std::string const &inFileName) {
    std::unique_ptr<samplesCommon::FileLock> fileLock{
        new samplesCommon::FileLock(inFileName)};
    std::ifstream iFile(inFileName, std::ios::in | std::ios::binary);
    if (!iFile) {
        sample::gLogWarning
            << "Could not read timing cache from: " << inFileName
...
inline void saveTimingCacheFile(std::string const &outFileName,
    ...
    std::unique_ptr<samplesCommon::FileLock> fileLock{
        new samplesCommon::FileLock(outFileName)};
    std::ofstream oFile(outFileName, std::ios::out | std::ios::binary);
    if (!oFile) {
        sample::gLogWarning
            << "Could not write timing cache to: " << outFileName << std::endl;
        return;
    }
...
inline void updateTimingCacheFile(std::string const &fileName,
    ...
    std::unique_ptr<samplesCommon::FileLock> fileLock{
        new samplesCommon::FileLock(fileName)};
    ...
    if (iFile) {
        ...
        sample::gLogInfo << "Loaded " << fsize << " bytes of timing cache from "
                         << fileName << std::endl;
        fileTimingCache.reset(config->createTimingCache(
            static_cast<const void *>(content.data()), content.size()));
        if (!fileTimingCache) {
            throw std::runtime_error("Failed to create timingCache from " +
                                     fileName + "!");
        }
    }
    ...
    if (!oFile) {
        sample::gLogWarning << "Could not write timing cache to: " << fileName
                            << std::endl;
        return;
    }
    oFile.write((char *)blob->data(), blob->size());
